...
test("signals paper mode produces target weights end-to-end", async ({ page }) => {
  ...
  // Switch to paper mode, confirm the switch landed (paper cash field only
  // renders in paper mode), then run explicitly — mode switch alone does not run.
  await page.getByRole("button", { name: "模拟资金" }).click();
  await expect(page.getByLabel("模拟资金")).toBeVisible();
  await page.getByRole("button", { name: "运行信号" }).click();
