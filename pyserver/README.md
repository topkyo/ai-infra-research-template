...
| 状态码 | 含义 | 调用方处理 |
|---|---|---|
| `400` | 输入校验失败（符号/日期不合法、`/analysts` 批量超过 50 个）。 | 修正请求后再试，不要原样重试。 |
| `404` | 标的/数据确认不存在（如 HK 无行情）。 | 可按“该标的不存在/无数据”处理。 |
| `502` | 上游暂时失败（详情见服务端日志）。 | 稍后重试或显式标记为 `unavailable`；**不得**把 502 当作“标的不存在”。 |
